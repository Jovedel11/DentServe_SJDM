...
import { useDoctorImageUpload } from "@/core/hooks/useDoctorImageUpload";
...
export const DoctorAvatar = ({
  ...
  return (
    <div className={`relative flex-shrink-0 ${className}`}>
      <div
        className={`${sizeClasses[size]} bg-muted rounded-full overflow-hidden flex items-center justify-center cursor-pointer group relative`}
        onClick={() =>
          document.getElementById(`doctor-file-${doctorId}`)?.click()
        }
      >
        {currentImage ? (
          <img
