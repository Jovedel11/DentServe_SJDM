...
export const useStaffNotifications = () => {
  ...
  // ✅ CORRECTED: Staff notification fetch with proper enum validation
  const fetchStaffNotifications = useCallback(async (options = {}) => {
    ...
    try {
      ...
      // ✅ CORRECTED: Proper data structure handling
  ...
  // ✅ CORRECTED: Use proper field names aligned with database response
  const statsAndFilters = useMemo(() => {
    ...
    // ✅ FIXED: Use 'type' field (database returns 'type', not 'notification_type')
    const byType = {
  ...
  // ✅ CORRECTED: Filter using proper field name 'type'
  const getFilteredNotifications = useCallback((filterType = 'all') => {
    switch (filterType) {
      ...
      case 'feedback':
        return notifications.filter(n => n.type === 'feedback_request');
      case 'partnerships':
