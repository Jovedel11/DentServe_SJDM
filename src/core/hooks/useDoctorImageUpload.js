...
export const useDoctorImageUpload = (doctorId, options = {}) => {
  ...
  const uploadFile = useCallback(async () => {
    ...
    if (!doctorId) {
      const error = 'Doctor ID is required';
      setError(error);
      return { success: false, error };
    }
    ...
    try {
      // ✅ FIXED: Create fresh FormData and add doctor ID first
      const formData = new FormData();
      formData.append('doctorId', doctorId);
      formData.append('doctorImage', fileToUpload);
      ...
      const result = await imageService.uploadDoctorImage(
        access_token,
        doctorId,
        formData,
      ...
      const successMessage = 'Doctor image uploaded successfully!';
  ...
  }, [compressedFile, originalFile, access_token, doctorId, onUploadStart, onUploadSuccess, onUploadError, cleanup]);
  ...
  return {
    ...
    canUpload: (compressedFile || originalFile) && !uploadState.isUploading && access_token && doctorId,
    selectFile,
    uploadFile,
    cancelUpload,
    reset,
