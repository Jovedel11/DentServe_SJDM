...
alter table "public"."otp_verifications" add constraint "otp_verifications_identifier_type_check" CHECK (((identifier_type)::text = ANY ((ARRAY['email'::character varying, 'phone'::character varying])::text[]))) not valid;

alter table "public"."otp_verifications" validate constraint "otp_verifications_identifier_type_check";
...
alter table "public"."patient_medical_history" add constraint "patient_medical_history_appointment_id_fkey" FOREIGN KEY (appointment_id) REFERENCES appointments(id) ON DELETE CASCADE not valid;
...
alter table "public"."staff_invitations" add constraint "staff_invitations_status_check" CHECK ((status = ANY (ARRAY['pending'::text, 'accepted'::text, 'expired'::text]))) not valid;
...
CREATE OR REPLACE FUNCTION public.check_rate_limit(p_user_identifier text, p_action_type text, p_max_attempts integer, p_time_window_minutes integer)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
CREATE OR REPLACE FUNCTION public.generate_otp(p_identifier character varying, p_identifier_type character varying, p_purpose character varying)
...
CREATE OR REPLACE FUNCTION public.get_ongoing_treatments(p_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
CREATE OR REPLACE FUNCTION public.send_phone_verification_otp(p_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
CREATE OR REPLACE FUNCTION public.verify_otp(p_identifier character varying, p_otp_code character varying, p_purpose character varying)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
CREATE OR REPLACE FUNCTION public.verify_phone_with_metadata(p_user_auth_id uuid, p_phone character varying)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog'
