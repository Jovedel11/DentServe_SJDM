...
create type "public"."notification_type" as enum ('appointment_reminder', 'appointment_confirmed', 'appointment_cancelled', 'feedback_request', 'partnership_request', 'feedback_response', 'admin_message');
...
create table "public"."appointments" (
    ...
    "requires_treatment_plan" boolean default false,
    "treatment_plan_notes" text,
    "treatment_plan_created_at" timestamp with time zone,
    "treatment_plan_created_by" uuid
);
...
create table "public"."patient_medical_history" (
    ...
    "treatment_plan_id" uuid,
    "requires_treatment_plan" boolean default false,
    "recommended_treatment_name" character varying(200),
    "recommended_visits" integer,
...
CREATE TABLE IF NOT EXISTS treatment_plan_cancellations (
    id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
    treatment_plan_id UUID NOT NULL REFERENCES treatment_plans(id) ON DELETE CASCADE,
    appointment_id UUID NOT NULL REFERENCES appointments(id) ON DELETE CASCADE,
    intended_visit_number INTEGER NOT NULL,
    booking_date TIMESTAMP WITH TIME ZONE,
    cancelled_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    cancelled_by UUID REFERENCES users(id),
    cancelled_by_role TEXT, -- 'patient', 'staff', 'admin'
    cancellation_reason TEXT,
    cancellation_type TEXT, -- 'patient_cancel', 'staff_reject', 'no_show'
    was_confirmed BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add indexes for performance
CREATE INDEX IF NOT EXISTS idx_treatment_cancellations_plan 
    ON treatment_plan_cancellations(treatment_plan_id);
    
CREATE INDEX IF NOT EXISTS idx_treatment_cancellations_appointment 
    ON treatment_plan_cancellations(appointment_id);
    
CREATE INDEX IF NOT EXISTS idx_treatment_cancellations_date 
    ON treatment_plan_cancellations(cancelled_at DESC);

-- Enable RLS
ALTER TABLE treatment_plan_cancellations ENABLE ROW LEVEL SECURITY;

-- RLS Policies
CREATE POLICY "Staff can view cancellations for their clinic"
    ON treatment_plan_cancellations FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM treatment_plans tp
            JOIN staff_profiles sp ON tp.clinic_id = sp.clinic_id
            JOIN user_profiles up ON sp.user_profile_id = up.id
            WHERE tp.id = treatment_plan_cancellations.treatment_plan_id
            AND up.user_id = (SELECT id FROM users WHERE auth_user_id = auth.uid())
        )
    );

CREATE POLICY "Patients can view their own cancellations"
    ON treatment_plan_cancellations FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM treatment_plans tp
            WHERE tp.id = treatment_plan_cancellations.treatment_plan_id
            AND tp.patient_id = (SELECT id FROM users WHERE auth_user_id = auth.uid())
        )
    );

COMMENT ON TABLE treatment_plan_cancellations IS 'Tracks cancelled/rejected treatment plan appointments for history and analytics';
...
CREATE INDEX idx_appointments_requires_treatment_plan ON public.appointments USING btree (clinic_id, created_at DESC) WHERE ((requires_treatment_plan = true) AND (treatment_plan_created_at IS NULL) AND (status = 'completed'::appointment_status));
...
CREATE INDEX idx_medical_history_treatment_needed ON public.patient_medical_history USING btree (patient_id, created_at DESC) WHERE (requires_treatment_plan = true);
...
CREATE INDEX idx_treatment_plans_source_appointment ON public.treatment_plans USING btree (source_appointment_id) WHERE (source_appointment_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_appointment_services_treatment_plan_id 
ON appointment_services(treatment_plan_id) 
WHERE treatment_plan_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_appointment_services_appt_treatment 
ON appointment_services(appointment_id, treatment_plan_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_patient_active_appointment_per_day
...
alter table "public"."appointments" add constraint "appointments_treatment_plan_created_by_fkey" FOREIGN KEY (treatment_plan_created_by) REFERENCES users(id) not valid;

alter table "public"."appointments" validate constraint "appointments_treatment_plan_created_by_fkey";
...
alter table "public"."treatment_plans" add constraint "treatment_plans_assigned_doctor_id_fkey" FOREIGN KEY (assigned_doctor_id) REFERENCES doctors(id) not valid;

alter table "public"."treatment_plans" validate constraint "treatment_plans_assigned_doctor_id_fkey";
...
alter table "public"."treatment_plans" add constraint "treatment_plans_source_appointment_id_fkey" FOREIGN KEY (source_appointment_id) REFERENCES appointments(id) not valid;

alter table "public"."treatment_plans" validate constraint "treatment_plans_source_appointment_id_fkey";
...
CREATE POLICY "patients_view_own_treatment_plans"
ON treatment_plans
FOR SELECT
TO authenticated
USING (
...
-- ✅ Policy 2: Staff can view treatment plans from their clinic
CREATE POLICY "staff_view_clinic_treatment_plans"
ON treatment_plans
FOR SELECT
TO authenticated
USING (
  EXISTS (
    ...
    AND sp.clinic_id = treatment_plans.clinic_id
    AND sp.is_active = true
  )
);

-- ✅ Policy 3: Staff can insert treatment plans for their clinic
CREATE POLICY "staff_insert_clinic_treatment_plans"
ON treatment_plans
FOR INSERT
TO authenticated
WITH CHECK (
...
-- ✅ Policy 4: Staff can update treatment plans for their clinic
CREATE POLICY "staff_update_clinic_treatment_plans"
ON treatment_plans
FOR UPDATE
TO authenticated
USING (
...
-- ============================================
-- RLS POLICIES FOR TREATMENT PLAN APPOINTMENTS
-- ============================================

-- ✅ Policy 5: Patients can view their treatment plan appointments
CREATE POLICY "patients_view_own_treatment_appointments"
ON treatment_plan_appointments
FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM treatment_plans tp
    WHERE tp.id = treatment_plan_appointments.treatment_plan_id
    AND tp.patient_id = auth.uid()
  )
);

-- ✅ Policy 6: Staff can view treatment plan appointments from their clinic
CREATE POLICY "staff_view_clinic_treatment_appointments"
ON treatment_plan_appointments
FOR SELECT
TO authenticated
USING (
...
-- ✅ Policy 7: Staff can insert treatment plan appointments
CREATE POLICY "staff_insert_treatment_appointments"
ON treatment_plan_appointments
FOR INSERT
TO authenticated
WITH CHECK (
...
-- ✅ Policy 8: Staff can update treatment plan appointments
CREATE POLICY "staff_update_treatment_appointments"
ON treatment_plan_appointments
FOR UPDATE
TO authenticated
USING (
...
CREATE OR REPLACE FUNCTION public.admin_award_badge(p_clinic_id uuid, p_badge_id uuid, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.admin_remove_badge(p_award_id uuid, p_removal_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.admin_send_notification(p_user_id uuid, p_message text, p_title text DEFAULT 'Admin Notification'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
    ...
    INTO target_user_auth_id, target_user_email
    FROM users
    WHERE id = p_user_id;
    ...
    -- Note: The actual password update happens in Supabase Auth via the admin SDK
    -- This function just logs the action and validates permissions
    
    RETURN jsonb_build_object(
        'success', true,
        'message', 'Password reset logged successfully. Update password via Supabase Auth Admin API.',
        'user_email', target_user_email,
        'auth_user_id', target_user_auth_id
    );
...
BEGIN
    ...
    IF p_treatment_plan_id IS NOT NULL THEN
        ...
        -- ✅ FIX: Calculate next visit number based on COMPLETED visits, not MAX bookings
        next_visit_number := COALESCE(treatment_plan_record.visits_completed, 0) + 1;
        
        RAISE NOTICE 'Treatment plan follow-up: Next visit number will be % (based on % completed visits)', 
            next_visit_number, treatment_plan_record.visits_completed;
    END IF;

    -- Get services for treatment plan follow-up
    IF p_treatment_plan_id IS NOT NULL AND (p_service_ids IS NULL OR array_length(p_service_ids, 1) IS NULL) THEN
        WITH treatment_services AS (
            SELECT ARRAY_AGG(DISTINCT aps.service_id) as service_ids
            FROM appointment_services aps
            JOIN appointments a ON aps.appointment_id = a.id
            WHERE aps.treatment_plan_id = p_treatment_plan_id
            AND aps.service_id IS NOT NULL
            AND a.status = 'completed'
        )
        SELECT service_ids INTO p_service_ids
        FROM treatment_services
        WHERE service_ids IS NOT NULL;
        
        IF p_service_ids IS NULL OR array_length(p_service_ids, 1) IS NULL THEN
            WITH all_treatment_services AS (
                SELECT ARRAY_AGG(DISTINCT aps.service_id) as service_ids
                FROM appointment_services aps
                WHERE aps.treatment_plan_id = p_treatment_plan_id
                AND aps.service_id IS NOT NULL
            ...
            SELECT service_ids INTO p_service_ids
            FROM all_treatment_services
            WHERE service_ids IS NOT NULL;
        END IF;
        
        RAISE NOTICE 'Treatment plan follow-up: Retrieved % services from treatment plan %', 
            COALESCE(array_length(p_service_ids, 1), 0), p_treatment_plan_id;
    END IF;
    ...
    IF p_treatment_plan_id IS NOT NULL THEN
        ...
        is_consultation_only := (p_service_ids IS NULL OR array_length(p_service_ids, 1) IS NULL);
    ELSIF p_service_ids IS NULL OR array_length(p_service_ids, 1) IS NULL OR array_length(p_service_ids, 1) = 0 THEN
    ...
    IF p_treatment_plan_id IS NOT NULL AND p_service_ids IS NOT NULL AND array_length(p_service_ids, 1) > 0 THEN
        SELECT COALESCE(SUM(min_price), 0) INTO total_treatment_cost
        FROM services
        WHERE id = ANY(p_service_ids) AND is_active = true;
        
        total_estimated_cost := total_estimated_cost + total_treatment_cost;
    END IF;
    ...
    IF NOT is_consultation_only THEN
        ...
        END IF;
    ELSE
        SELECT COALESCE(default_consultation_duration, 30) INTO total_duration
        FROM clinics WHERE id = p_clinic_id;
    END IF;

    appointment_datetime := (p_appointment_date + p_appointment_time);
    ...
    IF appointment_datetime < (NOW() + (min_advance_hours || ' hours')::INTERVAL) THEN
        RETURN jsonb_build_object(
            ...
            'minimum_booking_time', NOW() + (min_advance_hours || ' hours')::INTERVAL
        );
    END IF;
    ...
    IF p_appointment_date > CURRENT_DATE + (max_advance_days || ' days')::INTERVAL THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', format('Appointments cannot be booked more than %s days in advance', max_advance_days),
            'maximum_booking_date', CURRENT_DATE + (max_advance_days || ' days')::INTERVAL
        );
    END IF;
    ...
    IF pending_count >= max_pending_appointments THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', format('You have reached the maximum of %s pending appointments', max_pending_appointments),
    ...
    FROM user_profiles
    WHERE user_id = patient_id_val;

    appointment_limit_check := check_appointment_limits(patient_id_val, p_clinic_id);
    IF NOT (appointment_limit_check->>'can_book')::boolean THEN
        RETURN appointment_limit_check;
    END IF;

    reliability_check := check_patient_reliability(patient_id_val);
    IF NOT (reliability_check->>'can_book')::boolean THEN
    ...
    BEGIN
        INSERT INTO appointments (
            ...
            CASE 
                WHEN booking_type_val != 'service_only'::appointment_booking_type 
                THEN doctor_consultation_fee 
                ELSE NULL 
            END
        )
        ...
        -- ✅ FIXED: Treatment plan linkage with correct visit number
        IF p_treatment_plan_id IS NOT NULL THEN
            UPDATE treatment_plans
            SET 
                next_visit_appointment_id = appointment_id,
                next_visit_date = p_appointment_date,
                updated_at = NOW()
            WHERE id = p_treatment_plan_id;
            
            -- ✅ FIX: Use calculated next_visit_number based on completed visits
            INSERT INTO treatment_plan_appointments (
                ...
                p_treatment_plan_id,
                appointment_id,
                next_visit_number,  -- ✅ Uses visits_completed + 1
                'Follow-up Visit',
                false
            );
            
            RAISE NOTICE 'Linked appointment % to treatment plan % as visit number %', 
                appointment_id, p_treatment_plan_id, next_visit_number;
        END IF;

        -- Notifications (keeping existing logic)
        DECLARE
            staff_notification_title TEXT;
            staff_notification_message TEXT;
            staff_user_ids UUID[];
            doctor_full_name TEXT;
        BEGIN
            doctor_full_name := COALESCE(
                validation_result.doctor_first_name || ' ' || validation_result.doctor_last_name,
                'Unassigned'
            );
            
            IF booking_type_val = 'treatment_plan_follow_up' THEN
                staff_notification_title := '🔄 New Treatment Plan Follow-Up';
                staff_notification_message := format(
                    'Treatment plan follow-up scheduled for %s at %s. Patient: %s, Doctor: %s',
                    p_appointment_date,
                    p_appointment_time,
            ...
            ELSE
                staff_notification_title := '📅 New Appointment Request';
                staff_notification_message := format(
                    'New %s appointment for %s at %s. Patient: %s, Doctor: %s',
                    CASE 
                        WHEN booking_type_val = 'consultation_only' THEN 'consultation'
                        WHEN booking_type_val = 'service_only' THEN 'service'
                        ELSE 'consultation with service'
                    END,
                    p_appointment_date,
                    p_appointment_time,
                    patient_profile.first_name || ' ' || patient_profile.last_name,
            ...
            SELECT ARRAY_AGG(u.id) INTO staff_user_ids
            ...
            IF staff_user_ids IS NOT NULL AND array_length(staff_user_ids, 1) > 0 THEN
                INSERT INTO notifications (user_id, notification_type, title, message, related_appointment_id)
                SELECT 
                    unnest(staff_user_ids),
                    'appointment_confirmed',
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE LOG 'book_appointment error: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
            RETURN jsonb_build_object(
                'success', false,
                'error', 'Failed to book appointment: ' || SQLERRM
            );
    END;
END;
$function$;
...
DECLARE
    ...
    treatment_visit_number INTEGER;
BEGIN
    current_context := get_current_user_context();
    ...
    BEGIN
        ...
        -- ✅ NEW: Archive treatment plan appointment to cancellation history
        IF linked_treatment_plan_id IS NOT NULL THEN
            INSERT INTO treatment_plan_cancellations (
                treatment_plan_id,
                appointment_id,
                intended_visit_number,
                booking_date,
                cancelled_by,
                cancelled_by_role,
                cancellation_reason,
            ...
            SELECT 
                ...
                'patient_cancel',
                appointment_record.status = 'confirmed'
            FROM treatment_plan_appointments tpa
            WHERE tpa.appointment_id = p_appointment_id
            AND tpa.is_completed = false;
            
            -- ✅ NEW: Delete from treatment_plan_appointments (clean up)
            DELETE FROM treatment_plan_appointments
            WHERE appointment_id = p_appointment_id
            AND is_completed = false;
            
            -- ✅ NEW: Clear treatment plan's next visit reference
            UPDATE treatment_plans
            SET 
            ...
            RAISE NOTICE 'Cleaned up treatment plan % appointment linkage for visit %', 
                linked_treatment_plan_id, treatment_visit_number;
        END IF;
        ...
        RETURN jsonb_build_object(
            ...
            'data', jsonb_build_object(
                ...
                'cancelled_by', p_cancelled_by,
                'cancellation_reason', p_cancellation_reason,
                'treatment_plan_cleaned', linked_treatment_plan_id IS NOT NULL,
                'appointment_details', jsonb_build_object(
                    'appointment_date', appointment_record.appointment_date,
                    'appointment_time', appointment_record.appointment_time,
                    'clinic_name', appointment_record.clinic_name,
                    ...
                    'hours_notice', ROUND(hours_until_appointment, 1)
                )
            )
        );
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE LOG 'cancel_appointment error for %: % (SQLSTATE: %)', 
                      p_appointment_id, SQLERRM, SQLSTATE;
            RETURN jsonb_build_object('success', false, 'error', 
                format('Cancellation failed: %s', SQLERRM));
    END;
END;
$function$;
;
...
BEGIN
    ...
    -- ✅ Check clinic operating hours (FIXED VERSION)
    ...
    IF clinic_operating_hours IS NOT NULL THEN
        ...
        day_of_week := LOWER(TRIM(to_char(p_appointment_date, 'Day')));
        day_hours := clinic_operating_hours->day_of_week;
        ...
        IF day_hours IS NOT NULL THEN
            ...
            BEGIN
                ...
                -- Check if appointment starts before opening or ends after closing
                IF p_appointment_time < open_time OR end_time > close_time THEN
                    RETURN FALSE;
                END IF;
            EXCEPTION
                WHEN OTHERS THEN
                    -- If time parsing fails, log and continue (allow booking)
                    RAISE LOG 'Error parsing operating hours: %', SQLERRM;
            END;
        END IF;
    END IF;
...
CREATE OR REPLACE FUNCTION public.check_appointments_feedback_status(p_appointment_ids uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
CREATE OR REPLACE FUNCTION public.complete_appointment(p_appointment_id uuid, p_completion_notes text DEFAULT NULL::text, p_services_completed uuid[] DEFAULT NULL::uuid[], p_follow_up_required boolean DEFAULT false, p_follow_up_notes text DEFAULT NULL::text, p_requires_treatment_plan boolean DEFAULT false, p_treatment_plan_notes text DEFAULT NULL::text, p_diagnosis_summary text DEFAULT NULL::text, p_recommended_treatment_name character varying DEFAULT NULL::character varying, p_recommended_visits integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
BEGIN
    ...
    -- ✅ Transaction
    BEGIN
        -- Update appointment (ENHANCED with treatment plan flags)
        UPDATE appointments 
        SET 
            status = 'completed',
            notes = COALESCE(
            ...
            -- NEW: Treatment plan flags
            requires_treatment_plan = p_requires_treatment_plan,
            treatment_plan_notes = p_treatment_plan_notes,
            updated_at = NOW()
        WHERE id = p_appointment_id;

        -- ✅ Insert/Update medical history (ENHANCED with treatment recommendations)
        IF p_completion_notes IS NOT NULL OR p_services_completed IS NOT NULL OR p_requires_treatment_plan THEN
            INSERT INTO patient_medical_history (
                ...
                follow_up_required,
                follow_up_date,
                -- NEW: Treatment plan recommendations
                ...
                p_requires_treatment_plan,
                p_recommended_treatment_name,
                p_recommended_visits,
                p_diagnosis_summary
            )
            RETURNING id INTO medical_history_id;
        END IF;
...
IF p_follow_up_required THEN
    INSERT INTO notifications (
        ...
        (current_context->>'user_id')::UUID,
        'admin_message',  -- ✅ FIXED: Use valid enum value
        'Follow-up Required',
...
END IF;
        ...
        -- NEW: Treatment plan needed notification for staff
IF p_requires_treatment_plan THEN
    INSERT INTO notifications (
        user_id, 
        notification_type, 
        title, 
        ...
        (current_context->>'user_id')::UUID,
        'admin_message',  -- ✅ FIXED: Use existing enum value
        'Treatment Plan Needed',
        format('Patient %s requires treatment plan creation. Recommended: %s (%s visits)',
               appointment_record.patient_name,
               COALESCE(p_recommended_treatment_name, 'Not specified'),
               COALESCE(p_recommended_visits::text, 'TBD')),
        p_appointment_id,
        NOW()
    );
END IF;

        -- ✅ Final return (ENHANCED with treatment plan info)
        RETURN jsonb_build_object(
            'success', true,
            ...
            'data', jsonb_build_object(
                ...
                -- NEW: Treatment plan indicators
                'requires_treatment_plan', p_requires_treatment_plan,
                'medical_history_id', medical_history_id,
                'completed_by', current_context->>'user_id',
                'completed_at', NOW()
            )
        );
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false, 
            'error', format('Failed to complete appointment: %s', SQLERRM),
            'details', SQLSTATE
        );
END;
$function$
...
CREATE OR REPLACE FUNCTION public.create_treatment_plan(
    p_patient_id uuid,
    p_clinic_id uuid,
    p_treatment_name character varying,
    p_description text DEFAULT NULL::text,
    p_treatment_category character varying DEFAULT NULL::character varying,
    p_total_visits_planned integer DEFAULT NULL::integer,
    p_follow_up_interval_days integer DEFAULT 30,
    p_initial_appointment_id uuid DEFAULT NULL::uuid,
    p_source_appointment_id uuid DEFAULT NULL::uuid,
    p_diagnosis text DEFAULT NULL::text,
    p_assigned_doctor_id uuid DEFAULT NULL::uuid,
    p_start_date date DEFAULT CURRENT_DATE,
    p_expected_end_date date DEFAULT NULL::date,
...
BEGIN
    ...
    INSERT INTO treatment_plans (
        ...
        COALESCE(p_assigned_doctor_id, source_appointment_doctor_id),
        p_additional_notes
    )
    ...
    IF p_initial_appointment_id IS NOT NULL THEN
        ...
        SET treatment_plan_id = v_treatment_plan_id
    ...
    IF p_source_appointment_id IS NOT NULL THEN
        ...
        SET 
            ...
            treatment_plan_created_by = (current_context->>'user_id')::UUID
        WHERE id = p_source_appointment_id;
        
        UPDATE patient_medical_history
        SET treatment_plan_id = v_treatment_plan_id
        WHERE appointment_id = p_source_appointment_id;
    END IF;
    ...
    INSERT INTO notifications (
        ...
        format('A treatment plan "%s" has been created for you. %s Please contact the clinic to schedule your first visit.',
               p_treatment_name,
               CASE 
                   WHEN p_total_visits_planned IS NOT NULL 
                   THEN format('This plan includes %s visits. ', p_total_visits_planned)
                   ELSE ''
               END),
        NOW()
    );
...
WHEN 'patient' THEN
    WITH patient_appointments AS (
        SELECT 
            ...
            c.id as clinic_id, c.name as clinic_name, c.address as clinic_address, c.phone as clinic_phone, c.email as clinic_email,
...
WHEN 'staff' THEN
    -- ✅ FIXED: Added booking_type, treatment_plan data, and consultation_fee_charged
    WITH staff_appointments AS (
        SELECT 
            a.id, a.patient_id, a.appointment_date, a.appointment_time, a.status,
            a.symptoms, a.notes, a.duration_minutes, a.booking_type, a.consultation_fee_charged,
            a.created_at,
            c.id as clinic_id, c.name as clinic_name, c.address as clinic_address, 
            c.phone as clinic_phone, c.email as clinic_email, c.cancellation_policy_hours as clinic_cancellation_policy_hours,
    ...
    appointment_services_agg AS (
        SELECT 
            aps.appointment_id,
            (array_agg(aps.treatment_plan_id))[1] as treatment_plan_id,
            jsonb_agg(jsonb_build_object(
                'id', s.id,
                'name', s.name,
                'duration_minutes', s.duration_minutes,
    ...
    -- ✅ FIXED: Get treatment plan details for staff
    treatment_plan_data AS (
        SELECT 
            ...
            CASE 
                WHEN tp.id IS NOT NULL THEN
                    jsonb_build_object(
                        ...
                        'treatment_name', tp.treatment_name,
                        'treatment_category', tp.treatment_category,
                        'progress_percentage', COALESCE(tp.progress_percentage, 0),
                        'visits_completed', COALESCE(tp.visits_completed, 0),
                        'total_visits_planned', tp.total_visits_planned,
                        'status', tp.status,
                        'start_date', tp.start_date,
                        'expected_end_date', tp.expected_end_date,
                        'visit_number', tpa.visit_number,
                        'is_follow_up', (tpa.id IS NOT NULL)
                    )
                ELSE NULL
            END as treatment_plan
        FROM staff_appointments sa
        LEFT JOIN treatment_plan_appointments tpa ON tpa.appointment_id = sa.id
        LEFT JOIN treatment_plans tp ON tp.id = tpa.treatment_plan_id
    )
    SELECT jsonb_build_object(
        'success', true,
        'data', jsonb_build_object(
            'appointments', COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    END,
                    'services', COALESCE(asa.services, '[]'::jsonb),
                    'treatment_plan', tpd.treatment_plan,
    ...
    LEFT JOIN treatment_plan_data tpd ON sa.id = tpd.appointment_id;
...
CREATE OR REPLACE FUNCTION public.get_appointments_needing_treatment_plans(p_clinic_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
BEGIN
    ...
    -- Get staff's clinic
    clinic_id_val := COALESCE(p_clinic_id, (current_context->>'clinic_id')::UUID);
    
    IF clinic_id_val IS NULL THEN
        RETURN jsonb_build_object('success', false, 'error', 'Clinic ID required');
    END IF;
    ...
    WHERE a.clinic_id = clinic_id_val
    AND a.status = 'completed'
    AND a.requires_treatment_plan = TRUE
    AND a.treatment_plan_created_at IS NULL;
    
    RETURN jsonb_build_object(
        'success', true,
        'total_count', total_count,
        'data', (
            SELECT COALESCE(jsonb_agg(appointment_data ORDER BY created_at DESC), '[]'::jsonb)
            FROM (
                SELECT jsonb_build_object(
                    'appointment_id', a.id,
                    'appointment_date', a.appointment_date,
                    ...
                    'treatment_plan_notes', a.treatment_plan_notes,
                    ...
                    'patient', jsonb_build_object(
                        ...
                        'phone', u.phone,
                        'email', u.email,
                        'age', EXTRACT(YEAR FROM AGE(up.date_of_birth))::INTEGER,
                        'gender', up.gender
                    ),
                    
                    -- Doctor details (pre-select for plan)
                    'doctor', jsonb_build_object(
                        'id', d.id,
                    ...
                    'medical_history', (
                        SELECT jsonb_build_object(
                            'id', pmh.id,
                            'diagnosis_summary', pmh.diagnosis_summary,
                            'recommended_treatment_name', pmh.recommended_treatment_name,
                            'recommended_visits', pmh.recommended_visits,
                            'treatment_notes', pmh.treatment_notes,
                            'follow_up_required', pmh.follow_up_required,
                            'conditions', pmh.conditions,
                            'allergies', pmh.allergies
                        )
                        FROM patient_medical_history pmh
                        WHERE pmh.appointment_id = a.id
                        ORDER BY pmh.created_at DESC
                        LIMIT 1
                    ),
                    
                    -- Services performed (context for treatment)
                    'services', COALESCE((
                        SELECT jsonb_agg(jsonb_build_object(
                            'id', s.id,
                            'name', s.name,
...
DECLARE
    ...
    start_time TIME;
    end_time TIME;
    is_clinic_closed BOOLEAN := false;
...
BEGIN
    ...
    IF p_service_ids IS NOT NULL AND array_length(p_service_ids, 1) > 0 THEN
        SELECT COALESCE(SUM(duration_minutes), 60) 
        INTO total_duration
        FROM services 
        WHERE id = ANY(p_service_ids) AND is_active = true;
    ...
    -- ✅ Get clinic and operating hours
    SELECT 
        c.id,
        c.name,
        c.operating_hours,
    ...
    IF NOT FOUND THEN
        RETURN jsonb_build_object(
            'success', false, 
            'error', 'Doctor is not assigned to any clinic'
        );
    END IF;
    ...
    -- ✅ Parse clinic operating hours for the selected day
    IF clinic_info.operating_hours IS NOT NULL AND 
       (clinic_info.operating_hours->day_of_week) IS NOT NULL THEN
        
        clinic_hours := clinic_info.operating_hours->day_of_week;
        
        -- Check if clinic is closed on this day
        IF (clinic_hours->>'is_closed')::BOOLEAN = true THEN
            is_clinic_closed := true;
            -- Set default times for response (even though closed)
            start_time := TIME '09:00:00';
            end_time := TIME '17:00:00';
        ELSE
            -- Extract actual operating hours
            BEGIN
                start_time := (clinic_hours->>'open')::TIME;
                end_time := (clinic_hours->>'close')::TIME;
        ...
        END IF;
    ELSE
        -- Default hours if operating_hours not configured
        start_time := TIME '09:00:00';
        end_time := TIME '17:00:00';
    END IF;
    
    -- ✅ Generate time slots (only if clinic is open)
    IF NOT is_clinic_closed THEN
...
time_slots AS (
    SELECT 
        (start_time + (slot_num * INTERVAL '30 minutes'))::TIME as slot_time
    FROM generate_series(0, 100) as slot_num
    WHERE 
        -- Slot must fit before closing
        (start_time + (slot_num * INTERVAL '30 minutes'))::TIME <= 
        (end_time - (total_duration || ' minutes')::INTERVAL)
    AND 
        -- Apply 2-hour advance rule for same-day bookings
        (
            -- Future dates: show all slots
            p_appointment_date > CURRENT_DATE 
            OR 
            -- Same day: apply 2-hour minimum
            (
                p_appointment_date = CURRENT_DATE 
                AND
                (start_time + (slot_num * INTERVAL '30 minutes'))::TIME >= 
                (
                    CASE 
                        -- Before opening: min slot = opening + 2 hours
                        WHEN CURRENT_TIME < start_time 
                        THEN (start_time + INTERVAL '2 hours')::TIME
                        -- After opening: min slot = now + 2 hours
                        ELSE (CURRENT_TIME + INTERVAL '2 hours')::TIME
                    END
                )
            )
        )
        ...
        SELECT 
            ...
            COUNT(*)
        INTO available_slots, slot_count
    ...
    RETURN jsonb_build_object(
        ...
        'slot_count', COALESCE(slot_count, 0),
        'clinic_info', jsonb_build_object(
            'id', clinic_info.id,
            'name', clinic_info.name,
            'is_closed', is_clinic_closed,
            'day_of_week', day_of_week,
            'opening_time', start_time::TEXT,
            'closing_time', end_time::TEXT,
            'timezone', COALESCE(clinic_info.timezone, 'Asia/Manila'),
            'min_advance_hours', 2
        )
    );
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false, 
            'error', 'Failed to get time slots: ' || SQLERRM
        );
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.get_doctor_feedback(p_doctor_id uuid, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog', 'extensions'
...
CREATE OR REPLACE FUNCTION public.get_ongoing_treatments(
  p_patient_id uuid DEFAULT NULL::uuid, 
  p_include_paused boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF (current_context->>'user_type') = 'patient' THEN
    patient_id_val := (current_context->>'user_id')::UUID;
  ELSIF (current_context->>'user_type') IN ('staff', 'admin') THEN
    IF p_patient_id IS NOT NULL THEN
      patient_id_val := p_patient_id;
    ELSE
      patient_id_val := NULL;  -- Staff viewing all treatment plans at their clinic
    END IF;
  ELSE
  ...
  WITH treatment_details AS (
    SELECT 
      ...
      -- Treatment history (ONLY active appointments)
      (
        SELECT jsonb_agg(
          jsonb_build_object(
        ...
        AND a2.status != 'cancelled'  -- ✅ Exclude cancelled
        LIMIT 5
      ) as recent_visits,
      ...
      (
        SELECT COUNT(*)
        FROM treatment_plan_cancellations tpc
        WHERE tpc.treatment_plan_id = tp.id
      ) as total_cancelled_attempts,
  ...
  AND a.status IN ('pending', 'confirmed')
    LEFT JOIN doctors d ON a.doctor_id = d.id
    WHERE (patient_id_val IS NULL OR tp.patient_id = patient_id_val)  -- ✅ FIXED
    AND (
      tp.status = 'active' 
      OR (p_include_paused AND tp.status = 'paused')
    )
    AND (
      (current_context->>'user_type') = 'patient'
      OR tp.clinic_id = (
        SELECT COALESCE(
          (current_context->>'clinic_id')::UUID,
          sp.clinic_id
        )
        FROM staff_profiles sp
        JOIN user_profiles up ON sp.user_profile_id = up.id
    ...
    'data', jsonb_build_object(
      'ongoing_treatments', COALESCE(jsonb_agg(
        jsonb_build_object(
          ...
          -- ✅ FLATTEN these fields to top level
          'progress_percentage', COALESCE(progress_percentage, 0),
          'visits_completed', COALESCE(visits_completed, 0),
          'total_visits_planned', total_visits_planned,
          'total_cancelled_attempts', COALESCE(total_cancelled_attempts, 0),
          ...
          'progress', jsonb_build_object(
            ...
            'completion_ratio', CASE 
              WHEN total_visits_planned > 0 THEN 
                ROUND((COALESCE(visits_completed, 0)::NUMERIC / total_visits_planned * 100), 1)
              ELSE NULL
            END
          ),
          'timeline', jsonb_build_object(
            ...
            'days_until_next_visit', CASE 
              WHEN next_appointment_date IS NOT NULL THEN 
                next_appointment_date - CURRENT_DATE
              ELSE NULL
            END
          ),
          'clinic', jsonb_build_object(
          ...
          'alert_level', CASE 
            ...
            WHEN next_appointment_date IS NOT NULL AND next_appointment_date <= CURRENT_DATE + INTERVAL '7 days' THEN 'soon'
            ELSE 'normal'
          END
        )
      ), '[]'::jsonb),
      'summary', jsonb_build_object(
        ...
        'completion_avg', COALESCE(AVG(progress_percentage), 0)
      )
    )
  ) INTO result
...
CREATE OR REPLACE FUNCTION public.get_patient_ongoing_treatments_for_booking(
    p_patient_id uuid, 
...
BEGIN
    ...
    -- Access control: Only patients can see their own treatments
    ...
    -- ✅ ENHANCED: Only return treatments that need booking
    -- Excludes treatments with pending/confirmed appointments
    SELECT jsonb_build_object(
        'success', true,
        'data', jsonb_build_object(
            ...
            'treatments', COALESCE(
                jsonb_agg(
                    jsonb_build_object(
                        ...
                        'requires_booking', true,  -- All results require booking
                        'has_active_appointment', false  -- None have active appointments (filtered out)
                    )
                ),
                '[]'::jsonb
            )
    ...
    FROM treatment_plans tp
    JOIN clinics c ON tp.clinic_id = c.id
    LEFT JOIN doctors d ON d.id = tp.assigned_doctor_id
    LEFT JOIN appointments next_appt ON next_appt.id = tp.next_visit_appointment_id
    WHERE tp.patient_id = p_patient_id
    AND tp.status = 'active'
    AND (p_clinic_id IS NULL OR tp.clinic_id = p_clinic_id)
    -- ✅ CRITICAL FILTER: Only treatments WITHOUT pending/confirmed appointments
    AND (
        tp.next_visit_appointment_id IS NULL 
        OR next_appt.id IS NULL
        OR next_appt.status NOT IN ('pending', 'confirmed')
    );
...
CREATE OR REPLACE FUNCTION public.reject_appointment(
    ...
    p_rejection_category rejection_category DEFAULT 'other'::rejection_category, 
    p_suggest_reschedule boolean DEFAULT true, 
    p_alternative_dates date[] DEFAULT NULL::date[]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    BEGIN
        ...
        IF linked_treatment_plan_id IS NOT NULL THEN
            ...
            RAISE NOTICE 'Cleaned up treatment plan % appointment linkage for visit % (rejected)', 
                linked_treatment_plan_id, treatment_visit_number;
        END IF;
        ...
        BEGIN
            INSERT INTO analytics_events (
                ...
                jsonb_build_object(
                    ...
                    'appointment_date', appointment_record.appointment_date,
                    'patient_id', appointment_record.patient_id,
                    'was_treatment_plan', linked_treatment_plan_id IS NOT NULL
                )
            );
        EXCEPTION
            WHEN undefined_table THEN
                RAISE LOG 'analytics_events table does not exist, skipping analytics';
            WHEN OTHERS THEN
                RAISE LOG 'Failed to log analytics for rejection: %', SQLERRM;
        END;
        ...
        RETURN jsonb_build_object(
            ...
            'data', jsonb_build_object(
                ...
                'treatment_plan_cleaned', linked_treatment_plan_id IS NOT NULL,
                'rejection_stats', jsonb_build_object(
                    'total_rejections_this_month', rejection_stats.total_rejections_this_month
                ),
...
CREATE OR REPLACE FUNCTION public.get_treatment_plan_booking_info(p_treatment_plan_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_catalog', 'extensions'
...
BEGIN
    ...
    -- Get treatment plan with doctor and clinic info
    SELECT 
        tp.id,
        tp.patient_id,
        tp.clinic_id,
        ...
        tp.total_visits_planned,
        tp.next_visit_date,
        tp.next_visit_appointment_id,
        tp.source_appointment_id,
        tp.assigned_doctor_id,
        tp.follow_up_interval_days,
        tp.status,
        tp.progress_percentage,
        tp.start_date,                     
    ...
    FROM treatment_plans tp
    JOIN clinics c ON tp.clinic_id = c.id
    LEFT JOIN doctors d ON tp.assigned_doctor_id = d.id
    WHERE tp.id = p_treatment_plan_id
    AND tp.status = 'active';
    ...
    IF NOT FOUND THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Treatment plan not found or not active'
        );
    END IF;
    
    -- Access control: Only patient or staff at clinic can access
    IF (current_context->>'user_type') = 'patient' THEN
        IF (current_context->>'user_id')::UUID != treatment_record.patient_id THEN
            RETURN jsonb_build_object('success', false, 'error', 'Access denied');
        END IF;
    ELSIF (current_context->>'user_type') = 'staff' THEN
        IF (current_context->>'clinic_id')::UUID != treatment_record.clinic_id THEN
            RETURN jsonb_build_object('success', false, 'error', 'Access denied');
        END IF;
    END IF;
    
    -- Calculate next visit number
    next_visit_number := COALESCE(treatment_record.visits_completed, 0) + 1;
    
    -- Get services from the source/initial appointments
    -- Strategy: Get services from the LAST completed appointment in this treatment plan
    -- This assumes consistent service type across visits
    ...
    treatment_services AS (
        ...
        JOIN appointment_services aps ON ta.id = aps.appointment_id
        JOIN services s ON aps.service_id = s.id
        WHERE s.is_active = true
    )
    SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
            ...
            'requires_consultation', ts.requires_consultation
        )
    ), '[]'::jsonb) INTO services_data
    FROM treatment_services ts;
    
    -- If no services found from previous appointments, try to get from source appointment
    IF services_data = '[]'::jsonb AND treatment_record.next_visit_appointment_id IS NOT NULL THEN
        SELECT COALESCE(jsonb_agg(
            jsonb_build_object(
                'id', s.id,
                'name', s.name,
        ...
        JOIN appointment_services aps ON a.id = aps.appointment_id
        JOIN services s ON aps.service_id = s.id
        WHERE a.id = treatment_record.next_visit_appointment_id
        AND s.is_active = true;
    END IF;

    IF services_data = '[]'::jsonb AND treatment_record.source_appointment_id IS NOT NULL THEN
    SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
            'id', s.id,
            'name', s.name,
    ...
    JOIN appointment_services aps ON a.id = aps.appointment_id
    JOIN services s ON aps.service_id = s.id
    WHERE a.id = treatment_record.source_appointment_id
    AND s.is_active = true
    AND aps.service_id IS NOT NULL;
    
    RAISE NOTICE 'Retrieved % services from source appointment %', 
        jsonb_array_length(services_data), treatment_record.source_appointment_id;
END IF;
    ...
    BEGIN
        SELECT 
            SUM((elem->>'min_price')::DECIMAL),
            SUM((elem->>'max_price')::DECIMAL),
            SUM((elem->>'duration_minutes')::INTEGER)
        INTO estimated_min_cost, estimated_max_cost, total_duration
        FROM jsonb_array_elements(services_data) elem;
        
        -- Add consultation fee if doctor assigned and available
        IF treatment_record.assigned_doctor_id IS NOT NULL AND treatment_record.doctor_available THEN
            estimated_min_cost := COALESCE(estimated_min_cost, 0) + COALESCE(treatment_record.consultation_fee, 0);
            estimated_max_cost := COALESCE(estimated_max_cost, 0) + COALESCE(treatment_record.consultation_fee, 0);
        END IF;
        ...
        result := jsonb_build_object(
            ...
            'data', jsonb_build_object(
                ...
                'treatment_plan', jsonb_build_object(
                    ...
                    'treatment_name', treatment_record.treatment_name,
                    'treatment_category', treatment_record.treatment_category,
                    'description', treatment_record.description,
                    'diagnosis', treatment_record.diagnosis,
                    'visits_completed', treatment_record.visits_completed,
                    'total_visits_planned', treatment_record.total_visits_planned,
                    'next_visit_number', next_visit_number,
                    'progress_percentage', treatment_record.progress_percentage,
                    'follow_up_interval_days', treatment_record.follow_up_interval_days,
                    'status', treatment_record.status,
                    'start_date', treatment_record.start_date,             
                    'expected_end_date', treatment_record.expected_end_date  
                ),
                
                -- Clinic info (pre-filled)
                'clinic', jsonb_build_object(
                    'id', treatment_record.clinic_id,
                    'name', treatment_record.clinic_name,
                    'address', treatment_record.clinic_address,
                    'phone', treatment_record.clinic_phone,
                    'cancellation_policy_hours', treatment_record.cancellation_policy_hours
                ),
                
                -- Doctor info (pre-filled)
                'doctor', CASE 
                    WHEN treatment_record.assigned_doctor_id IS NOT NULL THEN
                        jsonb_build_object(
                            'id', treatment_record.assigned_doctor_id,
                            'name', treatment_record.doctor_name,
                            'specialization', treatment_record.doctor_specialization,
                            'consultation_fee', treatment_record.consultation_fee,
                            'is_available', treatment_record.doctor_available,
                            'rating', treatment_record.doctor_rating
                        )
                    ELSE NULL
                END,
                
                -- Services (pre-filled from previous visits)
                'services', services_data,
                
                -- Recommended next visit date
                'recommended_date', treatment_record.next_visit_date,
                
                -- Estimated costs
                'pricing', jsonb_build_object(
                    'min_total', COALESCE(estimated_min_cost, 0),
                    'max_total', COALESCE(estimated_max_cost, 0),
                    'consultation_fee', COALESCE(treatment_record.consultation_fee, 0),
                    'total_duration_minutes', COALESCE(total_duration, 30)
                ),
                
                -- Booking metadata
                'booking_type', 'treatment_plan_follow_up',
                'can_book', treatment_record.doctor_available = true,
                'skip_selection_steps', true  -- ✅ Flag to go directly to confirmation
            )
        );
    END;
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Failed to get treatment plan booking info: ' || SQLERRM
        );
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.get_treatment_plan_history(
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE tp.id = p_treatment_plan_id
        AND (
            tp.patient_id = (current_context->>'user_id')::UUID
            OR EXISTS (
                SELECT 1 FROM staff_profiles sp
                JOIN user_profiles up ON sp.user_profile_id = up.id
                WHERE sp.clinic_id = tp.clinic_id
                AND up.user_id = (current_context->>'user_id')::UUID
            )
        )
    ) THEN
        RETURN jsonb_build_object('success', false, 'error', 'Access denied');
        ...
        'data', jsonb_build_object(
            'completed_visits', (
                SELECT jsonb_agg(jsonb_build_object(
                    ...
                    'appointment_date', a.appointment_date,
                    'appointment_time', a.appointment_time,
                    'completed_at', tpa.completed_at,
                    'visit_purpose', tpa.visit_purpose,
                    'completion_notes', tpa.completion_notes,
                    'appointment_status', a.status
                ) ORDER BY tpa.visit_number)
                FROM treatment_plan_appointments tpa
                JOIN appointments a ON tpa.appointment_id = a.id
                WHERE tpa.treatment_plan_id = p_treatment_plan_id
                AND tpa.is_completed = true
                AND a.status = 'completed'  -- ✅ Double-check it's actually completed
            ),
            'pending_visits', (
                SELECT jsonb_agg(jsonb_build_object(
                    'visit_number', tpa.visit_number,
                ...
                WHERE tpa.treatment_plan_id = p_treatment_plan_id
                AND tpa.is_completed = false
                AND a.status IN ('pending', 'confirmed')  -- ✅ FIXED: Exclude cancelled
            ),
            'cancelled_attempts', (
                SELECT jsonb_agg(jsonb_build_object(
                    'intended_visit_number', tpc.intended_visit_number,
                    'appointment_id', tpc.appointment_id,
                    'booking_date', tpc.booking_date,
                    'cancelled_at', tpc.cancelled_at,
                    'cancelled_by_role', tpc.cancelled_by_role,
                    'cancellation_type', tpc.cancellation_type,
                    'cancellation_reason', tpc.cancellation_reason,
                    'was_confirmed', tpc.was_confirmed
                ) ORDER BY tpc.cancelled_at DESC)
                FROM treatment_plan_cancellations tpc
                WHERE tpc.treatment_plan_id = p_treatment_plan_id
            )
        )
    ) INTO result;
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Failed to get treatment plan history: ' || SQLERRM
        );
END;
$function$;

CREATE OR REPLACE FUNCTION public.get_treatment_plan_details(p_treatment_plan_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_catalog', 'extensions'
...
BEGIN
  ...
  -- Step 2: Fetch nested JSON with ALL fields INCLUDING PATIENT
  SELECT jsonb_build_object(
    'success', true,
    'data', jsonb_build_object(
      'id', tp.id,
      'status', tp.status,
      'treatment_notes', tp.treatment_notes,
      ...
      'follow_up_interval_days', tp.follow_up_interval_days,
      'next_visit_date', tp.next_visit_date,
      'patient_id', tp.patient_id,
      'clinic_id', tp.clinic_id,
      'assigned_doctor_id', tp.assigned_doctor_id,
      'source_appointment_id', tp.source_appointment_id,
      'diagnosis', tp.diagnosis,
      'requires_follow_up', tp.requires_follow_up,
      'next_visit_appointment_id', tp.next_visit_appointment_id,
      ...
      -- Created by (staff/admin user)
      'created_by', jsonb_build_object(
        'id', creator_user.id,
        'email', creator_user.email,
        'name', creator_profile.first_name || ' ' || creator_profile.last_name,
        'first_name', creator_profile.first_name,
        'last_name', creator_profile.last_name
      ),

      -- ✅ RENAMED: Changed to 'visits' for consistency with frontend
      'visits', (
        SELECT jsonb_agg(
          jsonb_build_object(
            'id', tpa.id,
            ...
            'is_completed', tpa.is_completed,
            'completion_notes', tpa.completion_notes,
            'recommended_next_visit_days', tpa.recommended_next_visit_days,
              ...
              'services', (
                SELECT jsonb_agg(
                  jsonb_build_object(
                    ...
                    'min_price', s.min_price,
                    'max_price', s.max_price,
                    'treatment_price', s.treatment_price,
                    'follow_up_price', s.follow_up_price,
                    'duration_minutes', s.duration_minutes
                  )
                )
                FROM appointment_services aps
          ...
          ORDER BY tpa.visit_number ASC
        )
        FROM treatment_plan_appointments tpa
        JOIN appointments a ON a.id = tpa.appointment_id
  ...
  INTO result
  FROM treatment_plans tp
  LEFT JOIN clinics c ON c.id = tp.clinic_id
  LEFT JOIN users creator_user ON creator_user.id = tp.created_by_staff_id
  LEFT JOIN user_profiles creator_profile ON creator_profile.user_id = creator_user.id
  -- ✅ ADDED: Join patient user and profile
  LEFT JOIN users patient_user ON patient_user.id = tp.patient_id
  LEFT JOIN user_profiles patient_profile ON patient_profile.user_id = patient_user.id
  WHERE tp.id = p_treatment_plan_id
  AND (
    tp.patient_id = (current_context->>'user_id')::UUID
    OR (current_context->>'user_type') IN ('staff', 'admin')
  );

  -- Step 3: Return
  RETURN COALESCE(result, jsonb_build_object(
    'success', false,
    'error', 'Treatment plan not found or unauthorized'
  ));
END;
$$;
...
DECLARE
    ...
    hours_until_old_appointment NUMERIC;
BEGIN
    -- Get context
    current_context := get_current_user_context();
    IF NOT (current_context->>'authenticated')::boolean THEN
    ...
    CASE current_user_role
        WHEN 'patient' THEN
            ...
            hours_until_old_appointment := appointment_record.hours_until_appointment;
            IF hours_until_old_appointment < COALESCE(appointment_record.cancellation_policy_hours, 24) THEN
                RETURN jsonb_build_object(
                    'success', false,
                    'error', format('Cannot reschedule within %s hours of appointment', 
                                  COALESCE(appointment_record.cancellation_policy_hours, 24))
                );
            END IF;
        WHEN 'staff' THEN
            ...
            END IF;
        WHEN 'admin' THEN
            NULL; -- Admin can reschedule anything
        ELSE
            RETURN jsonb_build_object('success', false, 'error', 'Insufficient permissions');
    END CASE;
    ...
    IF appointment_record.status NOT IN ('pending', 'confirmed') THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', format('Cannot reschedule appointment with status: %s', appointment_record.status)
        );
    END IF;
    
    -- Validate new date is in future
    IF p_new_date < CURRENT_DATE OR 
       (p_new_date = CURRENT_DATE AND p_new_time <= CURRENT_TIME) THEN
        RETURN jsonb_build_object('success', false, 'error', 'New appointment time must be in the future');
    END IF;
    
    -- Check doctor availability for new slot
    availability_check := check_appointment_availability(
        appointment_record.doctor_id,
        p_new_date,
        p_new_time,
        appointment_record.duration_minutes,
    ...
    IF NOT availability_check THEN
        RETURN jsonb_build_object(
            ...
            'data', jsonb_build_object('suggested_action', 'choose_different_time')
        );
    END IF;
    
    -- Store old values
    old_date := appointment_record.appointment_date;
    old_time := appointment_record.appointment_time;
    ...
    BEGIN
        ...
        SET 
            ...
            notes = COALESCE(notes, '') || 
                   E'\n---\nRESCHEDULED: ' || NOW()::text ||
                   E'\nFrom: ' || old_date || ' ' || old_time ||
                   E'\nTo: ' || p_new_date || ' ' || p_new_time ||
                   E'\nBy: ' || current_user_role ||
                   CASE 
                       WHEN p_reschedule_reason IS NOT NULL THEN 
                           E'\nReason: ' || p_reschedule_reason
                       ELSE ''
                   END,
            updated_at = NOW()
        WHERE id = p_appointment_id;
        
        -- Update treatment plan next visit date if linked
        UPDATE treatment_plans
        SET 
            next_visit_date = p_new_date,
            updated_at = NOW()
        WHERE next_visit_appointment_id = p_appointment_id;
        
        -- Create notifications based on who rescheduled
        IF current_user_role = 'patient' THEN
            -- Notify staff
            INSERT INTO notifications (user_id, notification_type, title, message, related_appointment_id)
            SELECT 
                ...
                format('Patient %s rescheduled appointment from %s %s to %s %s. Reason: %s',
                       appointment_record.patient_name,
                       old_date, old_time,
                       p_new_date, p_new_time,
                       COALESCE(p_reschedule_reason, 'Not provided')),
                p_appointment_id
            FROM staff_profiles sp
            JOIN user_profiles up ON sp.user_profile_id = up.id
            JOIN users u ON up.user_id = u.id
            WHERE sp.clinic_id = appointment_record.clinic_id AND sp.is_active = true;
        ELSE
            -- Notify patient
            INSERT INTO notifications (user_id, notification_type, title, message, related_appointment_id)
            VALUES (
                appointment_record.patient_id,
                'appointment_confirmed'::notification_type,
                'Appointment Rescheduled',
                format('Your appointment has been rescheduled from %s %s to %s %s. %s',
                       old_date, old_time,
                       p_new_date, p_new_time,
                       COALESCE('Reason: ' || p_reschedule_reason, '')),
                p_appointment_id
            );
        END IF;
        ...
        RETURN jsonb_build_object(
            ...
            'data', jsonb_build_object(
                ...
                'new_date', p_new_date,
                'new_time', p_new_time,
                'rescheduled_by', current_user_role,
                'rescheduled_at', NOW(),
                'clinic_name', appointment_record.clinic_name,
                'doctor_name', appointment_record.doctor_name,
                'patient_name', appointment_record.patient_name,
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'reschedule_appointment error: %', SQLERRM;
        RETURN jsonb_build_object('success', false, 'error', 'Failed to reschedule: ' || SQLERRM);
    END;
END;
$$;
...
BEGIN
    ...
    IF existing_feedback_count >= 5 THEN
        RETURN jsonb_build_object(
            'success', false, 
...
BEGIN
    -- ✅ ALLOW NULL service_id for treatment plan links
    IF NEW.service_id IS NULL THEN
        -- Valid for treatment plan appointments without specific services
        RETURN NEW;
    END IF;
    
    -- Get the clinic from the appointment
...
for select
to authenticated
using (((get_current_user_role() = 'patient'::user_type) AND (patient_id = get_current_user_id())));
...
using ((EXISTS ( SELECT 1
   FROM (staff_profiles sp
     JOIN user_profiles up ON ((sp.user_profile_id = up.id)))
  WHERE ((up.user_id = auth.uid()) AND (sp.clinic_id = feedback.clinic_id) AND (sp.is_active = true)))));
...
CREATE TRIGGER trigger_update_treatment_on_completion
